...
function select() {
    ...
    const theater =document.querySelector("#cinemas").value; 
    if (SeatsCount.length === 0 ||SeatsCount===0) {
        alert("예매 인원을 선택해주세요");
    } else if(theater.length === 0){
			alert("극장을 선택해주세요");
    } else {
...
//    const coupon = document.getElementById("coupon-box")
   ...
   const selectedPaymentMethod = paymentMethodWidget.getSelectedPaymentMethod()
    ...
    const payObj = {
        orderId: obj.mcode +'_' + id,            
        orderName: obj.mname,                 
        successUrl: window.location.origin + "/YellowTeamProject/movie/complete",  
...
function pay(){
		...
		document.querySelector("#mname").innerHTML = obj.name
		document.querySelector("#movie_code").innerHTML =obj.mcode
		document.querySelector("#Numseats").innerHTML =obj.count
		const price = obj.count*obj.price;
		obj.total = price;
		modal.show();
		/*tossPayInit();*/
